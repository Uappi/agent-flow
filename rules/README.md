...
Product-scoped rules live under `ext/<product-id>/rules/` (e.g. `ext/uappi-v2/rules/specifics/`). They are **not** listed in this index. At dispatch, Maestro adds paths from `ext/<product>/rules/` whose `scope` matches the task when that product is in `activeProducts` (see `skills/dispatch.md`).
