...
3. **Resolve the Monday user name.** Query the authenticated Monday user. If the user name is available, sanitize it for a file name: lowercase, trim spaces, replace spaces with `-`, and remove characters outside `a-z`, `0-9`, `_`, and `-`. If the authenticated Monday user cannot be resolved, omit the user segment entirely; do not use a fallback name.

4. **Build the archive file name.** Generate a new markdown file name that starts with the review date/time from the local machine, followed by the sanitized Monday user name when available, then the MR/PR identifier and short topic. Run `date '+%Y-%m-%d_%H-%M'` immediately before naming the upload file — never guess, convert from UTC, or reuse prior values. Example with user: `2026-05-21_14-30-gabriel-code-review-MR-123-checkout-fix.md`. Example without user: `2026-05-21_14-30-code-review-MR-123-checkout-fix.md`.

5. **Prepare the timestamped upload file.** Immediately after the MR/PR review flow saves `.memory/docs/code-review/review-merge-<MR-OR-PR-ID>-<short-topic>.md`, upload that markdown content to the `Revisões automáticas` file column on the Monday item using the timestamped file name from step 4. If the upload mechanism requires a local file path with the final attachment name, create or copy a temporary `.md` file outside the review source path, upload it, and leave the canonical local review file unchanged.
