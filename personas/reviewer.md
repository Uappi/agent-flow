...
2. **MR/PR review mode.** If the trigger is `Revisar merge/MR`, read the task or issue link supplied by the user when present, read the MR/PR diff from the link supplied by the user using the matching provider MCP when configured (fallback to the available provider access path when MCP is unavailable), read `README.ai.md` when present, and analyze only the merge-base diff. Cover side effects, performance, regression, logs/traceability, unmet requirements, and security risks. Produce `templates/task/code-review.md`, save it to `.memory/docs/code-review/review-merge-<MR-OR-PR-ID>-<short-topic>.md`, and post the same filled template as a comment on the target MR/PR using the matching provider MCP when configured and allowed. Skip to step 9.
