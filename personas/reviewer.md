...
- Never skip Monday archiving when MR/PR review mode generates or regenerates the local review document. Export that generated document to the Monday task's `Revisões automáticas` doc column. Do not export unrelated manual local edits outside the review flow. If export fails, report the blocker instead of silently continuing.
