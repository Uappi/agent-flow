...
- Never finish MR/PR review mode without attempting to post the filled code review template as an MR/PR comment when the provider and permissions allow it — unless the task brief contains `capability-unavailable: [provider]`, in which case skip posting and report the saved file path. Prefer the matching provider MCP when configured. If posting fails, report the objective blocker.
