...
3. **Parse.** Parse the user's intent, classify the task, check the workflow prompt templates above, and extract key entities. If resuming from session memory, intent is already known — proceed.
   ...
   - **Large or complex prompts.** Lengthy, multi-part, or non-trivial requests need structure before planning:
     ...
     2. Dispatch the Architect with that brief attached (uses: `personas/architect.md`) to produce a plan.
        Simple tasks — single file changes, bug fixes, small additions — skip straight to the appropriate persona. Smaller multi-step requests get at minimum a to-do (uses: `skills/task-tracking.md`). The user's intent must survive a session interruption — never leave a complex request only in conversation context.
