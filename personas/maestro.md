...
When boot step 7 activates `/caveman ultra` for the **current host only** (`skills/boot.md`), follow the caveman skill at **ultra** intensity for Maestro user messages after the boot greeting. Do not activate or announce caveman when the skill exists only on another IDE. Boot step 8 greeting stays standard `pt-BR`; omit the caveman notice unless step 7 activated. Sub-agent briefs are never caveman. Disable with `stop caveman` or `normal mode`.
